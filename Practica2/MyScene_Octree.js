...
class MyScene extends THREE.Scene {
    constructor (myCanvas) {
      ...
      //Usaremos un Octree para las colisiones
      ...
      this.model2 = new GeneraEscenario(10,this.octree);
      ...
      //Añadimos al personaje al Octree
      this.octree.add(this.model3,{useFaces:true });
    }
    ...
    update () {
      ...
      //Y tras ello, actualiza el octree
      this.octree.update();
    }
  ...
  checkColisiones(){
    var octreeObjects = this.octree.search(this.model3,1,true);
    if(octreeObjects.length>0){
      var distancias = [] ;
      for (let i=0 ; i<octreeObjects.length ; i++){
        var difx = this.model3.position.x - octreeObjects[i].object.position.x ;
        var dify = this.model3.position.y - octreeObjects[i].object.position.y ;
        var difz = this.model3.position.z - octreeObjects[i].object.position.z ;
        var modulo = Math.sqrt(difx*difx+dify*dify+difz*difz);
        distancias[i] = modulo ;
        if(modulo < 0.1) this.estado = MyScene.DEATH ;
      }
      distancias.sort(function(a,b){return b-a;});
      console.log(distancias[0]);
    }
  }
}
